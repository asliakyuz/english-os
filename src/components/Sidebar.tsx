...
import { BookOpen, MessageSquare, LayoutDashboard, CheckSquare, Calendar, Archive, ShieldCheck, BarChart3, FileText } from 'lucide-react';

interface SidebarProps {
  // Yeni eklediğimiz 'general_grammar' ve 'books_tracker' sekmelerini tiplere güvenle ekliyoruz
  activeTab: 'vocabulary' | 'archive' | 'oxford' | 'quiz' | 'journal' | 'goals' | 'grammar' | 'dashboard' | 'books_tracker' | 'general_grammar';
  setActiveTab: (tab: 'vocabulary' | 'archive' | 'oxford' | 'quiz' | 'journal' | 'goals' | 'grammar' | 'dashboard' | 'books_tracker' | 'general_grammar') => void;
  activeDays: number[];
}

export default function Sidebar({ activeTab, setActiveTab, activeDays }: SidebarProps) {
  // Menü elemanlarını tam istediğin gibi güncelledik ve yeni sekmeleri ekledik
  const menuItems = [
    { name: 'Performance Dashboard', id: 'dashboard', icon: <BarChart3 size={16} /> },
    ...
    { name: 'Essential Grammar in Use', id: 'books_tracker', icon: <LayoutDashboard size={16} /> }, // Akordeonlu Kitap Alanı
    { name: 'Genel Gramer Konuları', id: 'general_grammar', icon: <FileText size={16} /> }, // Bağımsız Manuel Alan
  ...
  return (
    <div className="w-64 h-screen bg-[#130c25] border-r border-[#231742] flex flex-col justify-between p-4 fixed left-0 top-0 z-50 overflow-y-auto">
      <div className="space-y-6">
        ...
        {/* TRACKER TAKVİM (Milisaniyesi bile değişmedi, tıkır tıkır çalışmaya devam ediyor) */}
